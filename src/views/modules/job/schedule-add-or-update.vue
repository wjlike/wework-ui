<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px">
      ...
      <el-form-item label="方法名称" v-if="falsee" prop="methodName">
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/timer/schedule/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/timer/schedule/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

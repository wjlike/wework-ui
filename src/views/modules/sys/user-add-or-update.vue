<template>
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
      ...
      <el-form-item label="用户账号" prop="usercode">
        <el-input v-model="dataForm.usercode" placeholder="用户帐号"></el-input>
      </el-form-item>
...
<script>
	...
	import ISelect from '@/views/common/select';
	export default {
		data() {
      var validateUserCode = (rule, value, callback) => {
        if(!this.dataForm.id && !/\S/.test(value)) {
          ...
          if(value.length > 50){
            callback(new Error('用户账号长度不得超过50位'))
          }else{
            callback()
          }
        }
			...
			var validateUserName = (rule, value, callback) => {
				if(!this.dataForm.id && !/\S/.test(value)) {
					...
					if(value.length > 50){
						callback(new Error('用户名长度不得超过50位'))
					}else{
						callback()
					}
				}
			...
			var validatePassword = (rule, value, callback) => {
				if(!this.dataForm.id && !/\S/.test(value)) {
					...
					if(value.length > 50){
						callback(new Error('密码长度不得超过50位'))
					}else{
						callback()
					}
				}
